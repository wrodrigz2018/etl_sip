...
class Command(BaseCommand):
    help = "Run ProteinJournal staging ETL from Excel to SQL Server destination"
    ...
    def add_arguments(self, parser) -> None:
        ...
        parser.add_argument(
            "--excel-path",
            default="",
            help="Path to Excel file (defaults to ETL_PROTEIN_JOURNAL.EXCEL_PATH)",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **options) -> None:
        ...
        audit = ETLRunAudit(
            ...
            dry_run=dry_run,
            status_message="Ejecutando ETL ProteinJournal Staging",
            is_running=True,
            progress_percent=10,
            summary_json={
                "etl_type": "protein_journal_staging",
        ...
        try:
            ...
            audit.status_message = "ETL ProteinJournal Staging completada"
            ...
            audit.status_message = "ETL ProteinJournal Staging fallida"
            audit.progress_percent = 100
            audit.is_running = False
            audit.ended_at = timezone.now()
            self._save_audit(audit)
    ...
    @staticmethod
    def _build_config(excel_path_override: str = "") -> tuple[ProteinJournalETLConfig, str]:
        etl_settings = getattr(settings, "ETL_PROTEIN_JOURNAL", None)
        if not etl_settings:
            raise CommandError("ETL_PROTEIN_JOURNAL settings are missing")
        ...
        for path in required_paths:
            ...
            for key in path:
                if not isinstance(current, dict) or key not in current:
                    dotted = ".".join(path)
                    raise CommandError(f"Missing ETL_PROTEIN_JOURNAL setting: {dotted}")
                current = current[key]
            if current in (None, ""):
                dotted = ".".join(path)
                raise CommandError(f"Empty ETL_PROTEIN_JOURNAL setting: {dotted}")
