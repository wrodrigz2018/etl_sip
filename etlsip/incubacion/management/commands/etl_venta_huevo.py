...
class Command(BaseCommand):
    help = "Run Venta Huevo Incubable ETL from SQL Server source to SQL Server destination"
    ...
    @staticmethod
    def _build_config() -> VentaHuevoETLConfig:
        etl_settings = getattr(settings, "ETL_VENTA_HUEVO", None)
        if not etl_settings:
            raise CommandError("ETL_VENTA_HUEVO settings are missing")
        ...
        for path in required_paths:
            ...
            for key in path:
                if not isinstance(current, dict) or key not in current:
                    dotted = ".".join(path)
                    raise CommandError(f"Missing ETL_VENTA_HUEVO setting: {dotted}")
                current = current[key]
            if current in (None, ""):
                dotted = ".".join(path)
                raise CommandError(f"Empty ETL_VENTA_HUEVO setting: {dotted}")
