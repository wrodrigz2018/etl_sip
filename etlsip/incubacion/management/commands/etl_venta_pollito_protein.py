...
from incubacion.services import (
    ...
    VentaPollitoProteinETLConfig,
    run_etl_venta_pollito_protein,
)


class Command(BaseCommand):
    help = "Run Venta de Pollito ETL from Protein (SQL Server) source to SQL Server destination"
    ...
    def handle(self, *args, **options) -> None:
        ...
        try:
            result = run_etl_venta_pollito_protein(
                config=config,
                start_date=start_date,
                end_date=end_date,
                dry_run=dry_run,
    ...
    @staticmethod
    def _build_config() -> VentaPollitoProteinETLConfig:
        etl_settings = getattr(settings, "ETL_VENTA_POLLITO_PROTEIN", None)
        if not etl_settings:
            raise CommandError("ETL_VENTA_POLLITO_PROTEIN settings are missing")
        ...
        for path in required_paths:
            ...
            for key in path:
                if not isinstance(current, dict) or key not in current:
                    dotted = ".".join(path)
                    raise CommandError(f"Missing ETL_VENTA_POLLITO_PROTEIN setting: {dotted}")
                current = current[key]
            if current in (None, ""):
                dotted = ".".join(path)
                raise CommandError(f"Empty ETL_VENTA_POLLITO_PROTEIN setting: {dotted}")
        ...
        return VentaPollitoProteinETLConfig(
            source=source,
