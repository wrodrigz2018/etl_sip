...
from incubacion.services import IncubacionETLConfig, SqlServerConnectionConfig, run_etl


class Command(BaseCommand):
    help = "Run incubacion ETL from SQL Server source to SQL Server destination"
    ...
    @staticmethod
    def _build_config() -> IncubacionETLConfig:
        etl_settings = getattr(settings, "ETL_INCUBACION", None)
        if not etl_settings:
            raise CommandError("ETL_INCUBACION settings are missing")
        ...
        for path in required_paths:
            ...
            for key in path:
                if not isinstance(current, dict) or key not in current:
                    dotted = ".".join(path)
                    raise CommandError(f"Missing ETL_INCUBACION setting: {dotted}")
                current = current[key]
            if current in (None, ""):
                dotted = ".".join(path)
                raise CommandError(f"Empty ETL_INCUBACION setting: {dotted}")

        source_cfg = etl_settings["SOURCE"]
        destination_cfg = etl_settings["DESTINATION"]
        source_codes = tuple(etl_settings.get("SOURCE_CODES") or ())

        if not source_codes:
            raise CommandError("ETL_INCUBACION.SOURCE_CODES must contain at least one value")
