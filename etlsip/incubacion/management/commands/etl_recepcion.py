...
class Command(BaseCommand):
    help = "Run Recepcion ETL from SQL Server source to SQL Server destination"
    ...
    @staticmethod
    def _build_config() -> RecepcionETLConfig:
        etl_settings = getattr(settings, "ETL_RECEPCION", None)
        if not etl_settings:
            raise CommandError("ETL_RECEPCION settings are missing")
        ...
        for path in required_paths:
            ...
            for key in path:
                if not isinstance(current, dict) or key not in current:
                    dotted = ".".join(path)
                    raise CommandError(f"Missing ETL_RECEPCION setting: {dotted}")
                current = current[key]
            if current in (None, ""):
                dotted = ".".join(path)
                raise CommandError(f"Empty ETL_RECEPCION setting: {dotted}")
        ...
        return RecepcionETLConfig(
            source=source,
            destination=destination,
            source_table=etl_settings["SOURCE_TABLE"],
            destination_table=etl_settings["DESTINATION_TABLE"],
            destination_date_column=etl_settings.get("DESTINATION_DATE_COLUMN", "Fecha_envio"),
            egg_trans_code=int(etl_settings.get("EGG_TRANS_CODE", 17)),
            facility_type=int(etl_settings.get("FACILITY_TYPE", 1)),
        )
